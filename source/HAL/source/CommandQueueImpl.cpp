...
namespace HAL {
    ...
    auto CommandQueue::Internal::Signal(Fence const& fence, std::optional<uint64_t> value) const -> void {   
       ...
       uint64_t pSignalSemaphoreValues[] = { value.value_or(fence.GetCompletedValue()) }; //TODO
    ...
    auto CommandQueue::Internal::Wait(Fence const& fence, std::optional<uint64_t> value) const -> void {
        ...
        uint64_t pWaitSemaphoreValues[] = { value.value_or(fence.GetCompletedValue()) }; //TODO
...
namespace HAL {
   ...
   // auto TransferCommandQueue::ExecuteCommandList() const -> void {
   // }
