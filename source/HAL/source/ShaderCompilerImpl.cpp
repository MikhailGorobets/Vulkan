...
auto HAL::ShaderCompiler::Internal::CompileFromString(std::wstring const& data, std::wstring const& entryPoint, ShaderStage target, std::vector<std::wstring> const& defines) const -> std::optional<std::vector<uint8_t>> {
...
auto HAL::ShaderCompiler::Internal::CompileFromFile(std::wstring const& path, std::wstring const& entryPoint, ShaderStage target, std::vector<std::wstring> const& defines) const -> std::optional<std::vector<uint8_t>> {
     ComPtr<IDxcBlobEncoding> pDxcSource;
     ThrowIfFailed(m_pDxcUtils->LoadFile(path.c_str(), nullptr, &pDxcSource));
     return CompileShaderBlob(pDxcSource, entryPoint, target, defines);
}

auto HAL::ShaderCompiler::Internal::CompileShaderBlob(ComPtr<IDxcBlobEncoding> pDxcBlob, std::wstring const& entryPoint, ShaderStage target, std::vector<std::wstring> const& defines) const -> std::optional<std::vector<uint8_t>> {
